  ...
  (let ((c (aref grid (car start) (cdr start)))
    ...
    (labels ((same-plant? (grid pos dir)
      ...
      (loop while states for state = (pop states)
            ...
            do (setf (gethash state visited) t)
               (loop for neighbor in neighbors when (not (gethash neighbor visited))
                     do (pushnew neighbor states :test #'equal))))
    (make-region :points visited :char c :area (hash-table-count visited) :perimeter perimeter :num-corners num-corners)))
  ...
  (let (regions visited)
    (loop for i from 0 below (array-dimension grid 0) do
      (loop for j from 0 below (array-dimension grid 1)
            for c = (aref grid i j)
            for pos = (cons i j)
            unless (some (lambda (ht) (gethash pos ht)) visited) ; or merge them somehow?
              do (push (find-region grid (cons i j)) regions)
                 (push (region-points (first regions)) visited)))
    regions))
